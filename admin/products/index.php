...
if(isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'delete')
{
	...
	$result = $product->setStatus($productID,9);
	if($result)
	{
		#deleted
		$feedback = "Product delete successful!";
		header("location:?notifMsg=".base64_encode($feedback)."&notifType=".base64_encode("success"));
	}
	else
	{
		#error occurred
		$feedback = "An error occurred while trying to delete Product!";
        ...
        <div id="container">
            ...
            <div style="margin-bottom:15px; height: 30px; clear:both; margin-top:10px">
                ...
                <div style="clear:both;float:right;font-size:18px;margin-top:-25px">
                    <a href="<?php echo __BASE_URL__ ;?>admin/products/new.php">Add New</a>
                </div>
            ...
            <table width="100%" height="155">
            	<thead style="background-color:#969; color:#FFF; text-align:center">
                	<tr>
                    	<td width="266">Product Name</td>
                    	<td width="266">Product Description</td>
                    	<td width="160">Product Status</td>
            ...
            ?>
               ...
               <tr>
                   <td style="text-decoration:underline">
                     <a href="<?php echo __BASE_URL__ ;?>admin/products/view.php?productID=<?php echo $allProduct['productID'] ; ?>"><?php echo $allProduct['productName'] ; ?></a>
                    </td> 
                   <td>
                     <?php echo $allProduct['productDescription'] ; ?>
                    </td> 
                   <td>
				   		<?php 
							foreach($product->getProperty('statusTitle') as $key=>$value)
							{
								echo ($allProduct['productStatus'] == $key) ?$value : "";
							}
						?>
                   </td>
                   <td>
                   	<a href="<?php echo __BASE_URL__ ;?>admin/products/edit.php?productID=<?php echo $allProduct['productID'] ; ?>">Edit</a>
                    <a onclick="return confirm('Are you sure you want to delete <?php echo $allProduct['productName'] ; ?>?')" href="?mode=delete&productID=<?php echo $allProduct['productID'] ; ?>"> | Delete</a>
                    </td>
                </tr>
                </tbody>
			<?php
